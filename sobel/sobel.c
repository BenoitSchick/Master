...
void init_sobel_arrays(int width , int height) {
	...
	// sobel result stock� hors du cache --> 140 cycles (cache) � 79 cycles (hors cache)
...
void sobel_complete(unsigned char *source, short threshold)
{
	...
	for (y = 1 ; y < img_height ; y++) {
		...
		//On r�cup�re une seule fois le pointeur vers chaque ligne correspondant � y-1, y et y+1
		...
		for (x = 1 ; x < img_width ; x++) {
			...
			//On parcours les colonnes correspondant � x-1, x, x+1 pour chaque ligne y-1, y, y+1 --> convolue le filtre de Sobel sur l'image
								...
								int x0_subimg, int y0_subimg, int w_subimg, int h_subimg)
{
	...
	for (y = y_start ; y < y_end ; y+=1) {
		...
		unsigned char *dst = &sobel_result[y0]; //selectionne la ligne y
											//	--> parcourt les pixels de x_start � x_end de la ligne y
		...
		//Interpolation
			...
			/*if (x+1 < x_end)       sobel_result[y0 + x + 1] = val;        // copie pixel � droite
			if (y+1 < y_end) {
