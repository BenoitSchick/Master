...
--	bit 2 => IRQ ack 
	...
	begin
		if reset_n = '0' then
			...
			if avalon_cs = '1' and avalon_wr = '1' then
				...
				if dma_state_s = IDLE then -- Remise à zéro de l'ACK uniquement quand le DMA est revenu à IDLE
					ack_s <= '0';
				end if;
			end if;
		end if;
		...
		if reset_n = '0' then
			...
			if avalon_cs = '1' and avalon_rd = '1' then
				case avalon_address is
					...
					when "011" 	=> avalon_read_data <= std_logic_vector(reg_size_img);
	...
	begin
		if reset_n = '0' then
			...
			case lcd_state_s is
				when IDLE_LCD	=>
					if dma_state_s = WRITE_DMA_LCD and lcd_busy_s = '0' then
						lcd_state_s	<= S1;
					end if;
	...
	lcd_busy_s <= '1' when lcd_state_s /= IDLE_LCD else '0';
	LCD_data	<= lcd_data_avalon_s when lcd_data_sel_s = '0' else lcd_data_dma_s;
	LCD_RDn		<= '1';
	...
	begin
		if reset_n = '0' then
			...
			case dma_state_s is
				when IDLE			=>
					offset_img_s			<= (others => '0'); --remise à zero des compteur pour le transfert de la prochaine image
					counter_offset_img_s 	<= (others => '0');
					lcd_data_dma_s			<= (others => '0');
				

					if start_transfert_s = '1' and ack_s = '0' and lcd_busy_s = '0' then	-- Start transfer image to DMA
				...
				when END_TR			=>
					...
					if dma_state_prev_s /= END_TR then -- une seule IRQ !
        				dma_done_s <= '1';
						dma_state_prev_s <= END_TR;
					else
						dma_done_s <= '0';
					end if;
